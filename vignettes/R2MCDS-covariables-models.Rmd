...
We can start with a simple example using the observations of `Alcidae` if the Gulf of St-Lawrence by observers from the Canadian Wildlife Service <span style="font-family:Courier New"> Eastern Canadian Seabirds at Sea_ (ECSAS) </span> monitoring program. We will use the different observers as a an explanatory variable and we run all four possible models. 
...
The main differences in the model the summary of model with explanatory variables appear in the table `Parameters of the detection function`. There will be more parameters in the table than if the model would have been fit without variables. The description of each parameter is under the table. 
