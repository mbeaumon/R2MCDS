...
vignette: >
  %\VignetteIndexEntry{Analysis including explanatory variables}
...
For analysis with covariables it will be important to set `monotone` option to 'none' otherwise the `distance.wrap()` function will return an error message. It is also important to note that it can take considerably more time to fit models with covariables.
