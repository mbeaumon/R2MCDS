...
If the user has collected the same data for many species and is interested in running the same model for many species, it is possible to do so via the lsub and split argument. The lsub argument consists of a list that contains all the species that the user want to include I the analysis and the split option indicate whether the species should be analyzed together (FALSE) or separately (TRUE).

For simplicity we will run the example with only one detection function (Half-normal key function with Cosine adjustment. However it is possible to test models with different key functions and subsequently chose the best model.   
...
It is also possible to correct the observed densities of a given rare species by using the detection curve of a similar species and making use of multipliers. As usual, the user can choose the key function and adjustment for the reference species or can simply let Distance pick the best model for the reference species (estimator=NULL).
